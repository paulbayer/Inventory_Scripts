...
import os, sys, pprint, boto3, datetime, logging
...
session_cp=boto3.Session(profile_name=pProfile)
...
for k in range(len(PipelineRun)):
	...
	for i in range(len(StateMachineArns)):
		...
		for j in range(len(response['executions'])):
			...
			if response['executions'][j]['startDate'] > PipelineStageStart and response['executions'][j]['stopDate'] < PipelineStageEnd:
		...
		for i in range(len(resp['Summaries'])):
			...
			if PipelineStart < resp['Summaries'][i]['CreationTimestamp']:
				...
				StackSetOperations.append({
					...
					'OperationDuration':resp['Summaries'][i]['EndTimestamp'] - resp['Summaries'][i]['CreationTimestamp']
				})
