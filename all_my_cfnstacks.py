...
for pregion in RegionList:
	...
	for profile in ProfileList: #Inventory_Modules.get_profiles(pProfiles,plevel,SkipProfiles):
		...
		try:
			...
			StackNum=len(Stacks['StackSummaries'])
			# pprint.pprint(Stacks)
			if verbose < 49:
				print(ERASE_LINE,"Profile: ",profile,"Region: ",pregion,"Found",StackNum,"Stacks",end='\r')
			logging.info(ERASE_LINE,"Profile: ",profile,"Region: ",pregion,"Found",StackNum,"Stacks")
			print(ERASE_LINE,"Profile: ",profile,"Region: ",pregion,"Found",StackNum,"Stacks",end='\r')
		...
		if len(Stacks['StackSummaries']) > 0:
			for y in range(len(Stacks['StackSummaries'])):
				StackName=Stacks['StackSummaries'][y]['StackName']
				StackStatus=Stacks['StackSummaries'][y]['StackStatus']
		...
		# 			VpcName="No name defined"
				...
				NumStacksFound += 1
print(ERASE_LINE)
print("Found",NumStacksFound,"Stacks across",NumProfilesInvestigated,"profiles across",NumRegions,"regions")
print()
