	...
	for region in gd_regions:
		...
		try:
			...
			if str(my_Error).find("security token included in the request is invalid") > 0:
				logging.error(
						f"Account #:{account['AccountId']} - The region you're trying ({region}) isn't enabled for your account")
				continue
		...
		try:
			print(
					f"{ERASE_LINE}Trying account {account['AccountId']} in region {region} -- {places_to_try} of {len(ChildAccounts) * len(gd_regions)}",
					end='\r')
...
print("We scanned {} accounts and {} regions totalling {} possible areas for resources.".format(len(ChildAccounts),
                                                                                                ...
                                                                                                len(ChildAccounts) * len(
		                                                                                                gd_regions)))
print("Found {} Invites across {} accounts across {} regions".format(len(all_gd_invites), len(ChildAccounts),
                                                                     len(gd_regions)))
print("Found {} Detectors across {} profiles across {} regions".format(NumObjectsFound, len(ChildAccounts),
                                                                       len(gd_regions)))
print()
...
if DeletionRun and (ReallyDelete or ForceDelete):
	...
	for y in range(len(all_gd_detectors)):
		...
		logging.warning(
				f"Detector {str(all_gd_detectors[y]['DetectorIds'][0])} has been deleted from child account {str(all_gd_detectors[y]['AccountId'])}")
		"""
