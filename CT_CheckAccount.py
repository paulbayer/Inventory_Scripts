...
print("Confirmed the role "+Fore.GREEN + role + Fore.RESET+" exists in account "+Fore.GREEN + pChildAccountId + Fore.RESET+" and trusts "+Fore.GREEN + "{}".format(calling_creds)+Fore.RESET + " within the Management Account")
...
if ProcessStatus[Step]['Success']:
	...
	print(ERASE_LINE+Fore.RED+"** Step 2 completed, but there were {} items found that couldn't be deleted".format(ProcessStatus[Step]['IssuesFound']-ProcessStatus[Step]['IssuesFixed'])+Fore.RESET)
else:
...
if ProcessStatus[Step]['Success']:
	...
	print(ERASE_LINE+Fore.RED+"** {} completed, but there were {} trail names found that couldn't be deleted".format(Step,ProcessStatus[Step]['IssuesFound']-ProcessStatus[Step]['IssuesFixed'])+Fore.RESET)
else:
...
if ProcessStatus[Step]['Success']:
	...
	print(ERASE_LINE+Fore.RED+"** {} completed, but there was {} blocker found that couldn't be fixed".format(Step,ProcessStatus[Step]['IssuesFound']-ProcessStatus[Step]['IssuesFixed'])+Fore.RESET)
else:
	print(ERASE_LINE+Fore.RED+"** {} completed with blockers found".format(Step)+Fore.RESET)
print()
...
try:
	print(Fore.BLUE + "{}:".format(Step) + Fore.RESET)
	print(" Checking account {} to make sure it's not already in a Control-Tower managed OU".format(pChildAccountId))
...
try:
	...
	for region in RegionList:
		...
		print(ERASE_LINE, "Checking account {} in region {} for SNS Topics".format(pChildAccountId, region), end='\r')
		...
		if len(SNSTopics) > 0:
			for x in range(len(SNSTopics)):
				...
				logging.info("Unfortunately, we've found an SNS Topic  for account %s in the %s region, which means we'll have to delete it before this account can be adopted.", pChildAccountId, region)
				ProcessStatus[Step]['Success'] = False
				ProcessStatus[Step]['IssuesFound']+=1
				SNSTopics2.append({
					'AccountId': pChildAccountId,
...
if ProcessStatus[Step]['Success']:
	...
	print(ERASE_LINE+Fore.RED+"** {} completed, but there were {} blockers found that couldn't be fixed".format(Step,ProcessStatus[Step]['IssuesFound']-ProcessStatus[Step]['IssuesFixed'])+Fore.RESET)
else:
	print(ERASE_LINE+Fore.RED+"** {} completed with blockers found".format(Step)+Fore.RESET)
print()
...
try:
	print(Fore.BLUE + "{}:".format(Step) + Fore.RESET)
	print(" Checking account {} for any Lambda functions containing the 'controltower' string".format(pChildAccountId))
	LambdaFunctions2 = []
	for region in RegionList:
		logging.error("Checking account %s in region %s for " + Fore.RED + "Lambda functions" + Fore.RESET, pChildAccountId, region)
		...
		if len(LambdaFunctions) > 0:
			logging.info(
				"Unfortunately, account %s contains %s functions with reserved names, which means we'll have to delete them before this account can be adopted.",	pChildAccountId, len(LambdaFunctions))
...
try:
	...
	RoleNames2=[]
	logging.error("Checking account %s for", pChildAccountId + Fore.RED+" Role names"+Fore.RESET)
	RoleNames=Inventory_Modules.find_role_names(account_credentials, 'us-east-1', ['controltower', 'ControlTower'])
	if len(RoleNames) > 0:
		logging.info("Unfortunately, account %s contains %s roles with reserved names, which means we'll have to delete them before this account can be adopted.",pChildAccountId,len(RoleNames))
...
print()
print("Checked {} steps and the results are:".format(NumOfSteps))
ChildAccountIsReady = True
NumberOfIssues = 0
NumberOfFixes = 0
for key in ProcessStatus.keys():
	ChildAccountIsReady = ChildAccountIsReady and ProcessStatus[key]['Success']
	NumberOfIssues = NumberOfIssues + ProcessStatus[key]['IssuesFound']
	NumberOfFixes = NumberOfFixes + ProcessStatus[key]['IssuesFixed']
	logging.info("%s Success marker is: %s and therefore our process result will be %s", key, ProcessStatus[key]['Success'], ChildAccountIsReady)
FixesWorked = (NumberOfIssues - NumberOfFixes == 0)
if ChildAccountIsReady and NumberOfIssues == 0:
	print(Fore.GREEN+"**** We've found NO issues that would hinder the adoption of this account ****"+Fore.RESET)
elif ChildAccountIsReady and FixesWorked:
