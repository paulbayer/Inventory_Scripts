	...
	for region in cfg_regions:
		...
		try:  # Looking for Configuration Recorders
			...
			if 'ConfigurationRecorders' in response.keys():
				for i in range(len(response['ConfigurationRecorders'])):
					...
					logging.info("Found another config recorder %s in account %s in region %s bringing the total found to %s ",
								 str(response['ConfigurationRecorders'][i]['name']),
								 account['AccountId'],
								 region,
								 str(NumObjectsFound))
			else:
		...
		try:  # Looking for Delivery Channels
			...
			if len(response['DeliveryChannels']) > 0 and response['DeliveryChannels'][0]['name'][-13:] != "DO-NOT-DELETE":
				...
				logging.info("Found another delivery channel %s in account %s in region %s bringing the total found to %s ",
							 str(response['DeliveryChannels'][i]['name']),
							 account['AccountId'],
							 region,
							 str(NumObjectsFound))
			else:
