...
def get_template_body_and_parameters(faws_acct, fExisting_stack_set_name):
	...
	cfn_client = faws_acct.session.client('cloudformation')
	stack_set_info = dict()
	try:
		stack_set_info = cfn_client.describe_stack_set(StackSetName=fExisting_stack_set_name)['StackSet']
		...
		print(ErrorMessage)
		stack_set_info['AccessError'] = True
...
def get_stack_ids_from_existing_stack_set(faws_acct, fExisting_stack_set_name):
	...
	try:
		...
		while 'NextToken' in response.keys():
			response = client_cfn.list_stack_instances(StackSetName=fExisting_stack_set_name, CallAs='SELF', NextToken=response['NextToken'])
...
def create_stack_set_with_body_and_parameters(faws_acct, fNew_stack_set_name, fStack_set_info):
	...
	client_cfn = faws_acct.session.client('cloudformation')
	return_response = dict()
	# TODO: We should change the template body to a template url to accomodate really big templates
	try:
...
def remove_stack_instances(faws_acct, fStack_instances, fOldStackSet):
	...
	logging.info(f"Deleting stacks from {fOldStackSet}")
	client_cfn = faws_acct.session.client('cloudformation')
	return_response = dict()
	regions = set()
	accounts = set()
...
def populate_new_stack_with_existing_stack_instances(faws_acct, fStack_instance_ids, fNew_stack_name):
	"""
	...
	stack_instance_ids = [stack_instance['StackId'] for stack_instance in fStack_instance_info['Summaries'] if
	                      stack_instance['Status'] in ['CURRENT', 'OUTDATED']]
	logging.info(f"Creating NEW stackset {fNew_stack_name} in account {faws_acct.acct_number} with stack_ids: {stack_instance_ids}")
	client_cfn = faws_acct.session.client('cloudformation')
	return_response = dict()
	try:
		response = client_cfn.import_stacks_to_stack_set(StackSetName=fNew_stack_name,
		                                                 StackIds=fStack_instance_ids,
		                                                 OperationPreferences={
			                                                 'RegionConcurrencyType'     : 'PARALLEL',
...
logging.info(f"Successfully connected to account {aws_acct.acct_number} to move stack instances from {pOldStackSet} to {pNewStackSet}")
...
if OldStackSetExists:
	if NewStackSetExists:
		...
		# OpId = populate_new_stack_with_existing_stack_instances(aws_acct, stack_info, pNewStackSet)
	else:
		"""
		...
		OpId = create_stack_set_with_body_and_parameters(aws_acct, pNewStackSet, stack_set_info)
		...
		while x < len(stack_ids['Stack_instances']):
			...
			x += limit
			print(f"Importing {len(stack_ids_subset)} into the new stackset now...")
