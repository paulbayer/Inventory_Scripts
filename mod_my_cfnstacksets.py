...
def setup_auth_and_regions(fProfile: str) -> (aws_acct_access, list):
	...
	print(
			f"\t\tWe'll also display those accounts in the stacksets that are no longer part of the organization") if pCheckAccount else ""
				...
				try:
					...
					if len(StackInstances) == 0 and pAccountModifyList is None and pRegionModifyList is None:
						...
						f_combined_stack_set_instances.append({
							...
							'PermissionModel'     : c_stackset_info[
								'PermissionModel'] if 'PermissionModel' in c_stackset_info else 'SELF_MANAGED',
						...
						if pAccountModifyList is None or StackInstance['Account'] in pAccountModifyList:
							...
							if pRegionModifyList is None or (StackInstance['Region'] in RegionList) or ChangesRequested:
								f_combined_stack_set_instances.append({
									...
									'DetailedStatus'      : StackInstance['StackInstanceStatus'][
										'DetailedStatus'] if 'DetailedStatus' in StackInstance[
										'StackInstanceStatus'] else None,
									'StatusReason'        : StackInstance[
										'StatusReason'] if 'StatusReason' in StackInstance else None,
									'OrganizationalUnitId': StackInstance[
										'OrganizationalUnitId'] if 'OrganizationalUnitId' in StackInstance else None,
									'PermissionModel'     : c_stackset_info[
										'PermissionModel'] if 'PermissionModel' in c_stackset_info else 'SELF_MANAGED',
									'StackSetName'        : c_stacksetname,
			...
			if verbose < 50:
				...
				for stack_instance in instances:
					if stack_instance['Account'] not in stack_instances.keys():
						stack_instances[stack_instance['Account']] = {'Region'     : [], 'DetailedStatus': stack_instance[
							'DetailedStatus'], 'StatusReason'                      : stack_instance['StatusReason'],
				...
				for k, v in stack_instances.items():
					...
					if verbose <= 30 and pOperationDate:
						...
						last_operation_date = cfn_client.describe_stack_set_operation(StackSetName=stack_set_name,
						                                                              OperationId=last_stackset_operation)[
...
def get_stack_set_deployment_target_info(faws_acct: aws_acct_access, fRegion: str, fStackSetName: str,
                                         fAccountRemovalList: list = None):
	"""
	Required Parameters:
...
def collect_cfnstacksets(faws_acct: aws_acct_access, fRegion: str, fChangesMade: bool = False) -> (dict):
	...
	# If accounts were specified and regions were specified
		...
		ApplicableStackSetInstancesList = list(
				filter(lambda n: (n['ChildAccount'] in pAccountModifyList and n['ChildRegion'] in pRegionModifyList),
...
def check_on_stackset_operations(OpsList: list, f_cfn_client):
	...
	while StillRunning:
		...
		for operation in OpsList:
			...
			StackSetStatus = f_cfn_client.describe_stack_set_operation(StackSetName=operation['StackSetName'],
...
def _modify_stacksets(StackSet_Dict: dict) -> dict:
	...
	if pdelete and len(applicable_stack_set_instances) > 0:
		...
		for StackSetName, StackSetData in StackSets.items():
			...
			StackInstanceItem += 1
			print(
					f"Now deleting stackset instances in {StackSetName}. {StackInstanceItem} of {len(ApplicableStackSetsList)} done")
			...
			# Section that handles the deletion for the "SELF_MANAGED" type of stackset
				...
				if pAccountModifyList is None:
					...
					if pRegionModifyList is None:
						...
						instances_to_modify = list(filter(
				...
				# They *did* specify some accounts,
					...
					instances_to_modify = list(filter(lambda n: (
							n['ChildAccount'] in pAccountModifyList and n['ChildRegion'] in pRegionModifyList and n[
						'StackSetName'] == StackSetName), applicable_stack_set_instances))
				else:  # Empty list
					instances_to_modify = []
				...
				if pConfirm or ReallyDelete:
					RemoveStackInstanceResult = _delete_stack_instances(aws_acct, pRegion, StackSetName, pRetain,
					                                                    AccountList, RegionsToDelete)
				else:
					RemoveStackInstanceResult = {'Success'     : False,
			...
			if RemoveStackInstanceResult['Success']:
				...
				Decision = (input(
						"Deletion of Stack Instances failed, but might work if we force it. Shall we force it? (y/n): ") in [
					            'y', 'Y'])
				if Decision:
					RemoveStackInstanceResult = _delete_stack_instances(aws_acct, pRegion, StackSetName, True,
					                                                    AccountList,
					                                                    RegionsToDelete)  # Try it again, forcing it this time
					if RemoveStackInstanceResult['Success']:
			...
			if RemoveStackSet:
				...
				# else if there WERE child stacks that were deleted
					...
					StackInstancesAreGone = Inventory_Modules.check_stack_set_status3(aws_acct, StackSetName,
...
def _delete_stack_instances(faws_acct: aws_acct_access, fRegion: str, fStackSetName: str, fRetain: bool,
                            fAccountList: list = None, fRegionList: list = None, fPermissionModel='SELF_MANAGED',
                            fDeploymentTargets=None) -> dict:
	"""
	Required Parameters:
	faws_acct - the object containing the account credentials and such
	fRegion - the region we're looking to make changes in
	...
	if ((fAccountList is None or fAccountList == []) and fPermissionModel.upper() == 'SELF_MANAGED') or (
		...
		return (return_response)
	try:
		delete_stack_instance_response = Inventory_Modules.delete_stack_instances3(faws_acct, fRegion, fRegionList,
		                                                                           fStackSetName, fRetain, StackSetOpId,
		                                                                           fAccountList, fPermissionModel,
		                                                                           fDeploymentTargets)
...
def _refresh_stacksets(StackSet_Dict: dict) -> dict:
	...
	for stackset in ApplicableStackSetsList:
		...
		if ReallyRefresh or pRetain:
			...
			if 'PermissionModel' in stacksetAttributes['StackSet'].keys() and stacksetAttributes['StackSet'][
...
if __name__ == '__main__':
	...
	if len(StackSet_Info['ApplicableStackSetInstancesList']) == 0 and len(
			StackSet_Info['StackSetNames']['StackSets']) > 0:
		# print()
		print(
	...
	# If the stacksets were not found...
		...
		print()
		print(
				f"Since this is a refresh, and we found no applicable stacksets with {pStackfrag}, that must mean the deletion worked!!")
