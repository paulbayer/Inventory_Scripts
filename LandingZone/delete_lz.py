...
if __name__ == "__main__":
    ...
    if AWS_SESSION_TOKEN_PASSED:
        client = boto3.client('organizations', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
    else:
        client = boto3.client('organizations', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
    ...
    if user_input == 'y':
        ...
        if AWS_SESSION_TOKEN_PASSED:
            client = boto3.client('stepfunctions', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY,                              aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
        else:
            client = boto3.client('stepfunctions', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY,                              aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
        ...
        while execution_status['status'] == 'RUNNING':
            wait_with_progress_bar(Message="Waiting for LandingZoneLaunchAVMStateMachine Delete event to complete", Seconds=30)
            execution_status = client.describe_execution(executionArn=execution_arn['executionArn'])
    ...
    if AWS_SESSION_TOKEN_PASSED:
        ...
        client = boto3.client('servicecatalog', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
    ...
    for stack_set_name in stack_set_names:
        ...
        if stack_set_exists:
            ...
            for instance in instances['Summaries']:
                if instance['Status'] == 'CURRENT' or instance['Status'] == 'OUTDATED' or instance['Status'] == 'INOPERABLE':
                    ...
                    delete_in_progress = True
                    while delete_in_progress:
                        operation_status = client.describe_stack_set_operation(StackSetName=stack_set_name,OperationId=response['OperationId'])
                ...
                while delete_in_progress:
                    ...
                    for operation in operations['Summaries']:
                        if operation['Action'] == "DELETE" and (
                                operation['Status'] != 'SUCCEEDED' and operation['Status'] != 'FAILED'): # In other words - the status is "Running"
                            delete_in_progress = True
    ...
    for account in accounts['Accounts']:
        if (account['Name'] == provided_account_name) and not (account['Id'] == '614019996801'):
            account_found = True
            if AWS_SESSION_TOKEN_PASSED:
                sts_client = boto3.client('sts', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
            else:
            ...
            s3_client = boto3.client('s3', region_name=AWS_REGION, aws_access_key_id=account_credentials['AccessKeyId'], aws_secret_access_key=account_credentials['SecretAccessKey'], aws_session_token=account_credentials['SessionToken'])
    ...
    for account in accounts['Accounts']:
        if (account['Name'] == provided_account_name):
            ...
            # create client with temporary credentials
            ec2_client = boto3.client('ec2', region_name=AWS_REGION, aws_access_key_id=account_credentials['AccessKeyId'], aws_secret_access_key=account_credentials['SecretAccessKey'], aws_session_token=account_credentials['SessionToken'])
    ...
    if AWS_SESSION_TOKEN_PASSED:
        s3_client = boto3.client('s3', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
    else:
        s3_client = boto3.client('s3', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
    # delete buckets
    ...
    for bucket in buckets:
        if ('aws-landing-zone-configuration' in bucket['Name']) or ('landingzonepipelineartifacts' in bucket['Name']):
            # empty the bucket and delete it
            if AWS_SESSION_TOKEN_PASSED:
                s3 = boto3.resource('s3', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
            else:
    ...
    if AWS_SESSION_TOKEN_PASSED:
        client = boto3.client('cloudformation', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
    else:
        client = boto3.client('cloudformation', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
    stacks = client.list_stacks()
    deleted_something = False
    ...
    if user_input == 'y':
        ...
        if AWS_SESSION_TOKEN_PASSED:
            session = boto3.session.Session(region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
        else:
            session = boto3.session.Session(region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
        # iterate through current step functions to find the ARN for the LandingZoneLaunchAVMStateMachine
        ...
        for region in guardduty_regions:
            if AWS_SESSION_TOKEN_PASSED:
                client = boto3.client('guardduty', region_name=region, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
            else:
                client = boto3.client('guardduty', region_name=region, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
        ...
        for account in accounts['Accounts']:
            if (account['Name'] == provided_account_name):
                ...
                # create client with temporary credentials
                session = boto3.session.Session(region_name=AWS_REGION, aws_access_key_id=account_credentials['AccessKeyId'], aws_secret_access_key=account_credentials['SecretAccessKey'], aws_session_token=account_credentials['SessionToken'])

                # iterate through current step functions to find the ARN for the LandingZoneLaunchAVMStateMachine
                # guardduty_regions = session.get_available_regions('guardduty')
                for region in guardduty_regions:
                    client = boto3.client('guardduty', region_name=region, aws_access_key_id=account_credentials['AccessKeyId'], aws_secret_access_key=account_credentials['SecretAccessKey'], aws_session_token=account_credentials['SessionToken'])
                    detectors = client.list_detectors()
                    for detector in detectors['DetectorIds']:
                        response = client.delete_detector(DetectorId=detector)
                        print("Deleted GuardDuty detector "+detector+" in region "+region)
        ...
        while delete_in_progress:
            ...
            for stack in stacks['StackSummaries']:
                ...
                if "(SO0044)" in desc and stack['StackStatus'] != 'DELETE_COMPLETE':
                    ...
                    delete_in_progress = True
            if delete_in_progress:
                wait_with_progress_bar(Message="Stack(s) are still being deleted (if it takes more than 1 hour, restart the process)", Seconds=30)
    ...
    print("\nStep 15 - Delete all the Landing Zone SSM Parameters")
    if AWS_SESSION_TOKEN_PASSED:
        client = boto3.client('ssm', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
    else:
        client = boto3.client('ssm', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
    ...
    print("\nStep 16 - Ensure the Landing Zone KMS Keys have been deleted")
    if AWS_SESSION_TOKEN_PASSED:
        client = boto3.client('kms', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, aws_session_token=AWS_SESSION_TOKEN)
    else:
        client = boto3.client('kms', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
