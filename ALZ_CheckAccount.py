...
if aws_account.AccountType.lower() == 'root' and pChildAccountId is None:
	...
	print(f"Since you didn't specify a specific account, we'll check all {len(aws_account.ChildAccounts)} accounts in the Org.")
elif pChildAccountId is None:
	sys.exit(f"Account {aws_account.acct_number} is a {aws_account.AccountType} account. This script should be run with Management Account credentials.")
else:
...
for childaccount in ChildAccountList:
	...
	try:
		...
		if str(my_Error).find("AuthFailure") > 0:
			...
			print(f"The child account MUST allow access into the necessary IAM roles from the Organization's Master Account for the rest of this script (and the overall migration) to run.")
		...
		if account_credentials['AccessKeyId'] is None:
			logging.error(f"Was {Fore.RED}not{Fore.RESET} able to successfully connect to account {childaccount} using credentials from account {aws_account.acct_number}... ")
			print()
			...
			logging.error(f"Was able to successfully connect to account {childaccount} using credentials from account {aws_account.acct_number}... ")
			print()
	...
	try:
		...
		for region in RegionList:
			print(ERASE_LINE, f"Checking account {childaccount} in region {region} for {Fore.RED}default VPCs{Fore.RESET}", end='\r')
	...
	for i in range(len(DefaultVPCs)):
		...
		if FixRun:
			logging.warning(f"Deleting VpcId {DefaultVPCs[i]['VPCId']} in account {DefaultVPCs[i]['AccountID']} in region {DefaultVPCs[i]['Region']}")
	...
	if ProcessStatus[childaccount]['Step1']['Success']:
		...
		print(f"{ERASE_LINE + Fore.GREEN}** Step 1 found {ProcessStatus[childaccount]['Step1']['IssuesFound']} issues, but they were fixed by deleting the default vpcs{Fore.RESET}")
		ProcessStatus[childaccount]['Step1']['Success'] = True
	elif (ProcessStatus[childaccount]['Step1']['IssuesFound'] > ProcessStatus[childaccount]['Step1']['IssuesFixed']):
		print(f"{ERASE_LINE + Fore.RED}** Step 1 completed, but there were {ProcessStatus[childaccount]['Step1']['IssuesFound'] - ProcessStatus[childaccount]['Step1']['IssuesFixed']} vpcs that couldn't be fixed{Fore.RESET}")
	else:
	...
	try:
		...
		logging.error(f"Checked account {childaccount} in {len(RegionList)} regions. Found {len(ConfigList)+len(DeliveryChanList)} issues with Config Recorders and Delivery Channels")
	except ClientError as my_Error:
	...
	if ProcessStatus[childaccount]['Step2']['Success']:
		...
		print(f"{ERASE_LINE + Fore.GREEN}** Step 2 found {ProcessStatus[childaccount]['Step2']['IssuesFound']} issues, but they were fixed by deleting the existing Config Recorders and Delivery Channels{Fore.RESET}")
		ProcessStatus[childaccount]['Step2']['Success'] = True
	elif (ProcessStatus[childaccount]['Step2']['IssuesFound'] > ProcessStatus[childaccount]['Step2']['IssuesFixed']):
		print(f"{ERASE_LINE + Fore.RED}** Step 2 completed, but there were {ProcessStatus[childaccount]['Step2']['IssuesFound'] - ProcessStatus[childaccount]['Step2']['IssuesFixed']} items found that couldn't be deleted{Fore.RESET}")
	else:
	...
	try:
		...
		for region in RegionList:
			...
			CTtrails = Inventory_Modules.find_cloudtrails(account_credentials, region, ['AWS-Landing-Zone-BaselineCloudTrail'])
			if len(CTtrails) > 0:
				logging.error(f"Unfortunately, we've found existing CloudTrails in account {childaccount} in the {region} region, which means we'll have to delete it before this account can be adopted.")
	...
	for i in range(len(CTtrails2)):
		logging.error(f"{Fore.RED}Found a CloudTrail trail for account {childaccount} in region {CTtrails2[i]['HomeRegion']} named {CTtrails2[i]['Name']}{Fore.RESET}")
		ProcessStatus[childaccount]['Step3']['IssuesFound'] += 1
		if FixRun:
			try:
				logging.error("CloudTrail trail deletion commencing...")
				delresponse = Inventory_Modules.del_cloudtrails(account_credentials, CTtrails2[i]['HomeRegion'], CTtrails2[i]['TrailARN'])
	...
	if ProcessStatus[childaccount]['Step3']['Success']:
		...
		print(f"{ERASE_LINE + Fore.GREEN}** Step 3 found {ProcessStatus[childaccount]['Step3']['IssuesFound']} issues, but they were fixed by deleting the existing CloudTrail trail names{Fore.RESET}")
		ProcessStatus[childaccount]['Step3']['Success'] = True
	elif (ProcessStatus[childaccount]['Step3']['IssuesFound'] > ProcessStatus[childaccount]['Step3']['IssuesFixed']):
		print(f"{ERASE_LINE + Fore.RED}** Step 3 completed, but there were {ProcessStatus[childaccount]['Step3']['IssuesFound'] - ProcessStatus[childaccount]['Step3']['IssuesFixed']} trail names found that couldn't be deleted{Fore.RESET}")
	else:
	...
	try:
		...
		for region in RegionList:
			print(f"{ERASE_LINE}Checking account {childaccount} in region {region} for {Fore.RED}GuardDuty{Fore.RESET}invitations", end='\r')
			...
			if len(GDinvites['Invitations']) > 0:
				...
				for x in range(len(GDinvites['Invitations'])):
					...
					logging.error(f"Unfortunately, we've found a GuardDuty invitation for account {childaccount} in the {region} region from account {GDinvites['Invitations'][x]['AccountId']}, which means we'll have to delete it before this account can be adopted.")
	...
	for i in range(len(GDinvites2)):
		logging.error(f"{Fore.RED}I found a GuardDuty invitation for account %s in region %s from account %s ", childaccount, GDinvites2[i]['Region'], GDinvites2[i]['AccountId']+Fore.RESET)
		ProcessStatus[childaccount]['Step4']['IssuesFound'] += 1
		if FixRun:
	...
	if ProcessStatus[childaccount]['Step4']['Success']:
		...
		print(f"{ERASE_LINE + Fore.GREEN}** Step 4 found {ProcessStatus[childaccount]['Step4']['IssuesFound']} guardduty invites, but they were deleted{Fore.RESET}")
		ProcessStatus[childaccount]['Step4']['Success'] = True
	elif (ProcessStatus[childaccount]['Step4']['IssuesFound'] > ProcessStatus[childaccount]['Step4']['IssuesFixed']):
		print(f"{ERASE_LINE + Fore.RED}** Step 4 completed, but there were {ProcessStatus[childaccount]['Step4']['IssuesFound'] - ProcessStatus[childaccount]['Step4']['IssuesFixed']} guardduty invites found that couldn't be deleted{Fore.RESET}")
	else:
	...
	if (childaccount in [d['AccountId'] for d in aws_account.ChildAccounts]):
		...
		print()
		print(f"Account # {childaccount} is not a part of the Organization. This account needs to be moved into the Organization to be adopted into the Landing Zone tool")
	...
	if ProcessStatus[childaccount]['Step5']['Success']:
		...
		print(f"{ERASE_LINE + Fore.GREEN}** Step 5 found {ProcessStatus[childaccount]['Step5']['IssuesFound']} issues, but we were able to move the account into the they were able to be fixed{Fore.RESET}")
		ProcessStatus[childaccount]['Step5']['Success'] = True
	elif (ProcessStatus[childaccount]['Step5']['IssuesFound'] > ProcessStatus[childaccount]['Step5']['IssuesFixed']):
		print(f"{ERASE_LINE + Fore.RED}** Step 5 completed, but there were {ProcessStatus[childaccount]['Step5']['IssuesFound'] - ProcessStatus[childaccount]['Step5']['IssuesFixed']} blockers found that couldn't be fixed{Fore.RESET}")
	else:
...
y.field_names = ['Account', 'Account Access', 'Default VPCs', 'Recorders', 'CloudTrail', 'GuardDuty', 'Org Member', 'Ready?']
for item in ProcessStatus:
	...
	x.add_row([item, ProcessStatus[item]['IssuesFound'], ProcessStatus[item]['IssuesFixed'], ProcessStatus[item]['ChildIsReady']])
	y.add_row([
		item,
		...
		ProcessStatus[item]['Step0']['Success'] and ProcessStatus[item]['Step1']['Success'] and ProcessStatus[item]['Step2']['Success'] and ProcessStatus[item]['Step3']['Success'] and ProcessStatus[item]['Step4']['Success'] and ProcessStatus[item]['Step5']['Success']
		])
print("The following table represents the accounts looked at, and whether they are ready to be incorporated into an ALZ environment.")
print(x)
print()
print("The following table represents the accounts looked at, and gives details under each type of issue as to what might prevent a successful migration of this account into an ALZ environment.")
print(y)
